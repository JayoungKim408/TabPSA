...
class RowColTransformer(nn.Module):
    def __init__(self, num_tokens, dim,  K, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col', alpha=1.0, beta=0.3, polynomial='chebyshev'):
        super().__init__()
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    PreNorm(dim, Residual(Attention(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout, alpha=alpha, beta=beta, polynomial=polynomial,  K=K))),
                    PreNorm(dim, Residual(FeedForward(dim, dropout = ff_dropout))),
                    PreNorm(dim*nfeats, Residual(Attention(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout, beta=beta, polynomial=polynomial, K=K))),
                    PreNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
                ]))
            else:
                self.layers.append(nn.ModuleList([
                    PreNorm(dim*nfeats, Residual(Attention(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout, polynomial=polynomial, K=K))),
                    PreNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
                ]))
...
class Transformer(nn.Module):
    def __init__(self, num_tokens, K, dim, depth, heads, dim_head, attn_dropout, ff_dropout, beta=1, polynomial='chebyshev'):
        super().__init__()
        self.layers = nn.ModuleList([])
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Residual(Attention(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout, beta=beta, polynomial=polynomial, K=K))),
                PreNorm(dim, Residual(FeedForward(dim, dropout = ff_dropout))),
            ]))
...
class simple_MLP(nn.Module):
    def __init__(self, dims):
        ...
        for i in range(num_layers):
            self.layers.append(nn.Linear(dims[i], dims[i+1]).to(torch.float32))
            self.layers.append(nn.SiLU())
        self.layers.append(nn.Linear(dims[-2], dims[-1]).to(torch.float32))
