...
def SAINT_pretrain(model, cat_idxs, X_train, y_train, opt, device, continuous_mean_std):
    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            _ , x_categ_enc, x_cont_enc = embed_data_mask(x_categ, x_cont, cat_mask, con_mask,model,vision_dset) # positive
            ...
            if 'contrastive' in opt.pt_tasks:
                aug_features_1  = model.transformer(x_categ_enc, x_cont_enc) # positive
                aug_features_2 = model.transformer(x_categ_enc_2, x_cont_enc_2) # negative
                ...
                if opt.pt_projhead_style == 'diff': # positive / negative에 같은 레이어 쓸 것이냐 (g_1, g_2 in the paper)
                    aug_features_1 = model.pt_mlp(aug_features_1) # positive
                    aug_features_2 = model.pt_mlp2(aug_features_2) # negative
                ...
                contrastive_loss   = opt.lam0*(loss_1 + loss_2)/2
            ...
            if 'denoising' in opt.pt_tasks:
                cat_outs, con_outs = model(x_categ_enc_2, x_cont_enc_2) # 이게 denoising 
                if len(con_outs) > 0:
                ...
                denoising_loss = opt.lam2*l1 + opt.lam3*l2    
                loss += denoising_loss
            loss.backward()
            optimizer.step()
            ...
            if opt.active_log:
                wandb.log({'iteration': iter, 'pretrain_iter_loss': loss})
                wandb.log({'iteration': iter, 'denoising_l1': l1,'denoising_l2': l2})
                wandb.log({'iteration': iter, 'contrastive': contrastive_loss,'contrastive_l2': loss_2})
                wandb.log({'iteration': iter, 'contrastive_l1': loss_1,'contrastive_l2': loss_2})
        training_end = time.time()
        training_times.append(training_end - training_start)
        if opt.active_log:
            wandb.log({'epoch': epoch, 'pretrain_epoch_loss': running_loss})
