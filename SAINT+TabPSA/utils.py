...
def classification_scores(model, dloader, device, task,vision_dset):
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            ...
            if task == 'binary':
                ...
                proba = torch.cat([proba,m(y_outs).float()],dim=0)
    ...
    if task == 'binary':
        ...
        auc = roc_auc_score(y_score=proba.cpu(), y_true=y_test.cpu(), multi_class="ovr", average="macro",)
    return acc.cpu().numpy(), auc, end-start
...
def r2(model, dloader, device, vision_dset):
    model.eval()
    y_test = torch.empty(0).to(device)
    y_pred = torch.empty(0).to(device)
    with torch.no_grad():
